...
% create elements
problem.elementTopologies = zeros(1,nQuad+1+nE);
problem.elementTypeIndices = zeros(1,nQuad+1+nE);
problem.elementNodeIndices = cell(nQuad+1+nE,1);
for e = 1:nQuad
    problem.elementTopologies(e) = 2;
    problem.elementTypeIndices(e) = 1;
    problem.elementNodeIndices{e} = [e, e+1, e+nN, e+1+nN];
end
for e = nQuad+1:nQuad+1+nE
...
counter = nQuad+2;
for e = 1:nE
    problem.elementNodeIndices{counter} = nN + [e, e+1];
    counter = counter + 1;
end
...
problem.loads = { @(X)[ 0.0; -traction(X(1)) ] };
problem.penalties = { [0, 1e60; 0, 1e60] };
problem.foundations = { };

problem.elementLoads(1:nQuad+1+nE) = { [] };
for e = nQuad+2:nQuad+1+nE
    problem.elementLoads{e} = 1;
end

problem.elementPenalties(1:nQuad+1+nE) = { [] };
problem.elementPenalties{nQuad+1} = 1;

problem.elementFoundations(1:nQuad+1+nE) = { [] };
