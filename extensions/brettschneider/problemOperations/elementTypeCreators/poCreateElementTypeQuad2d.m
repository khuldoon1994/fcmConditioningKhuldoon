function newType = poCreateElementTypeQuad2d( typeData )
%poCreateElementTypeQuad2d Summary of this function goes here
...
%   - "poCreateElementTypeLine1d.m"
    ...
    if numel(typeData)<8
       disp('ERROR! Not enough entries in type data for cell type STANDARD_QUAD_2D. Need 8.');
    end
    
    quadratureType.name = 'gaussLegendreTest';
    quadratureType.order = 2;
    quadratureType.setupQuadraturePoints = @eoSetupGaussLegendre2d;
...
%         disp('ERROR! No gaussOrder given for element type STANDARD_QUAD_2D. Assuming 2.');
%     else
%         p = typeData{2};
%     end
    ...
    if ~strcmp(typeData{3},'physics') || ~ischar(typeData{4})
        disp('ERROR! No physic given for element type STANDARD_QUAD_2D. Assuming PLANE_STRAIN.');
    else
    ...
    if ~strcmp(typeData{5},'youngsModulus') || ~isscalar(typeData{6})
        disp('ERROR! No youngsModulus given for element type LEGENDRE_QUAD_2D. Assuming 1.');
    else
        E = typeData{6};
    end
    ...
    if ~strcmp(typeData{7},'poissonRatio') || ~isscalar(typeData{8})
        disp('ERROR! No poissonRatio given for element type LEGENDRE_QUAD_2D. Assuming 0.');
    else
        nu = typeData{8};
    end
...
%     newType.quadratureType.setupQuadraturePoints = @gaussianQuadrature2d;
