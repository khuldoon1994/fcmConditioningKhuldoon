...
for i=1:length(problem.elementQuadratures)
    ...
    minmax=[-1 1];
    for j=1:2
    nodes(:,j) = elementType.mappingEvaluator(problem, i, minmax(j));
    end
    plot(nodes(2,:),nodes(1,:),'k-s','MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',12)
    ...
    if ~isempty(insideElements)
        for j=1:length(insideElements)
            ...
            for k=1:2
            nodes(:,k) = elementType.mappingEvaluator(problem, i, Rnodes(k));
            end
            plot(nodes(2,:),nodes(1,:),'b-+','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',10)
        end
    end
...
%     end
    ...
    if ~isempty(brokenElements)
        for j=1:length(brokenElements)
            ...
            plot(nodes(2,:),nodes(1,:),'r-+','MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',10)
        end
    end
end
...
for i=1:length(problem.elementQuadratures)
    ...
    plot(globalPoints(2,:),globalPoints(1,:),'kx','MarkerSize',6)
end
hold off
